...
│                         应用层 (Vue 3)                           │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌───────────┐ │
│  │ App.vue     │ │ Toolbar     │ │ StatusBar   │ │ Sidebar    │ │
│  │ (入口组件)   │ │ (工具栏)     │ │ (状态栏)    │ │ (大纲/文件)│ │
│  └─────────────┘ └─────────────┘ └─────────────┘ └───────────┘ │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐               │
│  │ Settings    │ │ Command     │ │ SearchBar   │               │
│  │ Modal       │ │ Palette     │ │ (搜索替换)   │               │
│  └─────────────┘ └─────────────┘ └─────────────┘               │
└─────────────────────────────────────────────────────────────────┘
                              │
...
│                        数据层                                    │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌───────────┐ │
│  │ Pinia Store │ │ MD 解析/序列化│ │ 微信渲染器   │ │ 微信主题   │ │
│  │ (file.ts)   │ │ (markdown.ts)│ │ (wechat-*)  │ │ (themes)  │ │
│  │ (settings.ts)│               │              │             │ │
│  └─────────────┘ └─────────────┘ └─────────────┘ └───────────┘ │
└─────────────────────────────────────────────────────────────────┘
                              │
...
│                      平台层 (Tauri 2.0)                          │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌───────────┐ │
│  │ 文件系统 API │ │ 系统对话框   │ │ 原生菜单事件  │ │ 多窗口支持 │ │
│  │ (Rust cmd)  │ │ (dialog)    │ │ (menu-event)│ │ (webview) │ │
│  └─────────────┘ └─────────────┘ └─────────────┘ └───────────┘ │
└─────────────────────────────────────────────────────────────────┘
```
...
| `createImageHandlePlugin` | `image-handle.ts` | 图片拖拽、粘贴、本地存储 |
| `smartPaste` | `smart-paste.ts` | 智能粘贴（URL 转链接等） |
| `createSearchPlugin` | `search.ts` | 搜索高亮与替换 |
| `createLinkTooltipPlugin` | `link-tooltip.ts` | 链接悬停提示 |
...
| NodeView | Vue 组件 | 功能 |
|----------|----------|------|
| `code_block` | `CodeBlockView.vue` | 代码块编辑、语言搜索选择、高亮 |
| `image` | `ImageView.vue` | 图片预览、缩放、相对路径解析 |
| `math_inline` | `MathView.vue` | 行内数学公式编辑与渲染 |
| `math_block` | `MathView.vue` | 块级数学公式编辑与渲染、悬停预览 |
...
interface Settings {
  ...
  tabSize: number;
  spellCheck: boolean;
  wechatTheme: string;    // 微信导出主题
}

// 特殊状态
- isFocusMode: boolean    // 焦点写作模式
```
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ 内容变更      │ ──▶ │ isDirty=true │ ──▶ │ 防抖定时器   │
│ (Transaction)│     │ (Pinia)      │     │ (可配置间隔) │
└──────────────┘     └──────────────┘     └──────────────┘
                            │
                            ▼
                     ┌──────────────┐
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ 打开文件夹    │ ──▶ │ list_directory│ ──▶ │ 文件列表显示  │
│ (dialog)     │     │ (Rust)       │     │ (Sidebar)    │
└──────────────┘     └──────────────┘     └──────────────┘
                            │
                            ▼
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ 右键菜单      │ ──▶ │ rename_file  │ ──▶ │ 刷新文件列表  │
│ 重命名/删除   │     │ delete_file  │     │ (refresh)    │
└──────────────┘     │ create_file  │     └──────────────┘
                     │ create_folder│
                     └──────────────┘
```
...
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ Cmd+Shift+P  │ ──▶ │ CommandPalette│ ──▶ │ 搜索过滤命令 │
│ 快捷键       │     │ (Vue 组件)    │     │ (fuzzy match)│
└──────────────┘     └──────────────┘     └──────────────┘
                            │
                            ▼
                     ┌──────────────┐
